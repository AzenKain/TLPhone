...
export class OrderService {
    ...
    private async getProductDetail(productId: number,  productVariantId: number, quantity: number): Promise<{ product: ProductEntity, variantIndex: number }>{
        ...
        const variantIndex = product.details.variants.findIndex(e => e.id == productVariantId)
        if (variantIndex == -1) {
            throw new NotFoundException(`Product Variant with ID ${productVariantId} not found`);
        }
        if (product.details.variants[variantIndex].stockQuantity - quantity < 0) {
            throw new ForbiddenException('Product is empty');
        }
        product.buyCount += quantity
        product.details.variants[variantIndex].stockQuantity -= quantity
    ...
    async CreateOrderService(dto: createOrderDto, user: UserEntity) {
        ...
        for (const product of dto.orderProducts) {
            ...
            await this.productRepository.save(productData.product)
